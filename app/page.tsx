...
const factionMap: Record<FactionKey, FactionInfo> = {
  VC: {
    ...
    short: "VC 阵营",
    slogan: "先定方向，再定秩序。",
    tone: "这一派更看重方向高度、组织边界和领导权威，通常目标感强、控制力高，适合高不确定环境下快速立中心。",
    tags: ["目标感强", "边界清晰", "中心稳定"],
    colors: {
  ...
  VF: {
    ...
    short: "VF 阵营",
    slogan: "先给方向，再给空间。",
    tone: "这一派更看重长期愿景、个体成长和组织弹性，通常开放度高、想象力强，适合新方向探索和非标准问题处理。",
  ...
  RC: {
    ...
    short: "RC 阵营",
    slogan: "先看结果，再谈过程。",
    tone: "这一派更重视交付质量、经营效率和组织纪律，通常执行强、结果意识强，适合复杂环境下建立可验证产出系统。",
    tags: ["结果导向", "执行力强", "秩序感高"],
    colors: {
  ...
  RF: {
    ...
    short: "RF 阵营",
    slogan: "看似松弛，实则有数。",
    tone: "这一派更偏长期经营、边界感和低波动管理，不依赖高压存在感，而是通过持续经营和节奏管理推动组织向前。",
...
const profileMap: Record<string, Profile> = {
  ...
  VCSM: {
    title: "价值收拢型 BOSS",
    tone: "你擅长在温和氛围中建立一致认知，并逐步把团队带入清晰的价值体系，适合长期文化与组织建设。",
    tags: ["价值观强", "风格温和", "组织收拢"],
  ...
  RFSM: {
    title: "长期掌柜型 BOSS",
    tone: "你不依赖高压和高戏剧性，而是通过经营意识、稳定节奏和长期布局推动组织发展，适合长期经营与组织维护。",
    tags: ["长期主义", "松弛经营", "边界清楚"],
...
export default function Page() {
  ...
  return (
    <div className="min-h-dvh bg-[radial-gradient(circle_at_top,_rgba(251,191,36,0.18),_transparent_30%),radial-gradient(circle_at_right,_rgba(239,68,68,0.14),_transparent_24%),linear-gradient(180deg,#fafaf9_0%,#fff7ed_100%)] px-4 py-6 md:px-8">
        ...
        {!started ? (
          <div className="rounded-[28px] bg-white p-5 shadow-sm ring-1 ring-slate-200 md:p-6">
            ...
            <p className="mt-4 text-sm leading-7 text-slate-700 md:text-base">
              这是一份 36 题正式版娱乐型管理风格评估。结果将从愿景/营收、控制/放权、戏剧/稳定、开干/开会四组维度，对你的管理偏好进行归类，并落入 16 种类型与 4 个派系之中。
            </p>

            <div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-4">
              {["36 道题", "16 型", "4 派系", "正式版结果页"].map((item) => (
                <div
                  key={item}
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                <p className="mt-2 text-sm leading-7 text-slate-700">
                  适合创业者、管理者、团队负责人、业务合伙人，以及想从更客观视角了解自己管理风格的用户。
                </p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          ...
          <Card className="rounded-[28px] bg-white shadow-sm ring-1 ring-slate-200">
              ...
              <div className="space-y-3">
                {currentQuestion.options.map((option, idx) => (
                  <button
                    ...
                    className="flex w-full items-start justify-between rounded-2xl border border-slate-200 bg-white px-4 py-4 text-left shadow-sm active:scale-[0.99]"
                    style={{ WebkitTapHighlightColor: "transparent" }}
                  >
                    <span className="pr-4 text-[15px] leading-7 text-slate-900 md:text-base">
                      {option.text}
                    </span>
                ...
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                  <div>
                    ...
                    <div className="mt-2 flex flex-wrap items-end gap-3">
                      <CardTitle className="text-2xl font-black text-slate-900 md:text-4xl">
                        {result.title}
                      </CardTitle>
              ...
              <CardContent className="space-y-6 pb-6">
                <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
                  {code.split("").map((letter) => (
                    ...
                    >
                      <div className="text-xl font-black text-slate-900 md:text-2xl">{letter}</div>
                ...
                >
                  ...
                  <div className="mt-3 grid gap-4 md:grid-cols-[0.9fr_1.1fr]">
                    <div
